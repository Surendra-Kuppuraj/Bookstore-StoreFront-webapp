...
export class UploadImageService {
  ...
  upload(bookId: number) {
    if( typeof this.filesToUpload !== 'undefined' && this.filesToUpload.length >0) {
      this.makeFileRequest(AppConst.serverPath+"/image/add?id="+bookId, [], this.filesToUpload).then(
      (result) => {
          console.log(result);
       }, 
       (error) => {
  ...
  modify(bookId: number) {
    ...
    if (this.filesToUpload.length>0) {
      this.makeFileRequest(AppConst.serverPath+"/image/updateImage?id="+bookId, [], this.filesToUpload).then((result) => {
      console.log(result);
    }, (error) => {
      console.log(error);
    });
